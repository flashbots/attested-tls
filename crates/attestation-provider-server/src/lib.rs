...
pub async fn attestation_provider_server(
    ...
    let app = axum::Router::new()
        ...
        .with_state(SharedState { attestation_generator });
...
async fn get_attest(
    ...
    let input_data: [u8; 64] =
        hex::decode(input_data)?.try_into().map_err(|_| ServerError::InvalidLength)?;

    let attestation = shared_state.attestation_generator.generate_attestation(input_data)?.encode();
...
mod tests {
    ...
    async fn test_attestation_provider_server() {
        ...
        tokio::spawn(async move {
            attestation_provider_server(listener, attestation_generator).await.unwrap();
        });
        attestation_provider_client(server_addr, AttestationVerifier::expect_none()).await.unwrap();
    }
}
