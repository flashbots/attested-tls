            ...
            Value::Array(records) => {
                ...
                for record in records_simple {
                    let attestation_type: AttestationType =
                        serde_json::from_value(Value::String(record.attestation_type.clone()))?;

                    let expected_measurements = match (
                        record.measurements,
                        record.dcap_image_hashes,
                    ) {
                        (Some(_), Some(_)) => {
                            ...
                            AttestationType::DcapTdx | AttestationType::GcpTdx => {
                                ExpectedMeasurements::Dcap(
                                    measurements
                                        ...
                                        .map(|(index_str, entry)| {
                                            Ok((
                                                DcapMeasurementRegister::from_policy_key(
                                                    index_str,
                                                )?,
                        ...
                        (None, Some(image_hashes)) => match attestation_type {
                            // Currently only GCP is supported for portable measurement policy -
                            // but support for other types is planned.
