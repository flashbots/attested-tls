...
//!
//! Implementors wrap a rustls [`Connection`] (client or server side) and
//! expose its RFC 5705 exporter output via a backend-agnostic trait.
//! Consumers feed the exported bytes into higher-layer attestation bindings
//! — typically by hashing them into a DCAP quote's `user_report_data` so
//! the quote is cryptographically bound to the specific TLS session that
//! produced the originating request.
//!
...
pub enum ExportError {
    /// The session cannot yet export — typically the handshake is
    /// incomplete.
    #[error("session is not ready to export keying material (handshake incomplete)")]
    HandshakeIncomplete,

    /// The underlying TLS library refused the export with a
    /// library-specific error. The wrapped string is the library's
...
///
/// One implementation wraps a real rustls session (behind the
/// `rustls-exporter` feature); tests supply deterministic fakes.
pub trait SessionExporter {
    /// Fill `out` with `out.len()` bytes of RFC 5705 exporter keying
    /// material, derived from the session's negotiated master secret
    /// and the caller- supplied `(label, context)` pair.
    ///
    /// Both sides of a TLS session that call this with the same `(label,
    /// context, out.len())` observe byte-identical output. This is the
    /// cryptographic property consumers rely on when using the exporter as
    /// a channel-binding input.
    ///
    /// # Errors
    ///
    /// - [`ExportError::HandshakeIncomplete`] if the session hasn't
...
mod rustls_impl {
    ...
    impl<D: SideData> SessionExporter for RustlsExporter<'_, D> {
        fn export_keying_material(
            ...
            self.conn.export_keying_material(out, label, context).map(|_| ()).map_err(|e| match e {
