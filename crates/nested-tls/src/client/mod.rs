...
/// A connector that performs an outer TLS handshake and then an inner TLS
...
impl NestingTlsConnector {
    /// Creates a connector from the outer and inner client TLS configs.
    pub fn new(outer: Arc<ClientConfig>, inner: Arc<ClientConfig>) -> Self {
        Self { outer: TlsConnector::from(outer), inner: TlsConnector::from(inner) }
    }

    /// Starts the two-stage nested TLS handshake against `domain` on `stream`.
    #[inline]
    pub fn connect<IO>(
        &self,
...
/// Future returned from `client::NestingTlsConnector::connect` which will
/// resolve once both outer and inner connection handshakes had finished.
